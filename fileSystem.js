...
function getAllFilePathsWithExtension(directoryPath, extension, filePaths) {
    ...
    for (const fileName of fileNames) {
        ...
        const filePath = directoryPath + matchDirectotyDelimeter() + fileName;
...
function matchDirectotyDelimeter() {
...
function getFileName(filePathsArr) {
    let filePathNameArr = [];
    for (let i=0;i<filePathsArr.length;i++){
        filePathNameArr[i]=path.basename(filePathsArr[i]);
    }
    return filePathNameArr;
}
